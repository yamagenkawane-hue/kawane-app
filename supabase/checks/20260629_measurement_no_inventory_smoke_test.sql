-- Smoke test for measurement registration.
--
-- This script writes inside a transaction and finishes with ROLLBACK, so it
-- does not leave test production_results, order_processes, or inventory changes.
--
-- Expected result:
-- - result = PASSED
...
begin
  ...
  if target_process.id is null then
    ...
    values (
      'SKIPPED',
      'ロットNoがあり、計量工程に残登録可能数がある対象データが見つかりませんでした'
    );
    return;
  end if;
  ...
  values (
    ...
    case
      when after_results = before_results + 1
       and after_stock = before_stock
      then '計量実績登録で production_results は更新され、inventory_items は更新されませんでした。ROLLBACKするため変更は残りません'
