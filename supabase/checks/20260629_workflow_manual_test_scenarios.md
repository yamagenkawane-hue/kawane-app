...
DB側の自己完結確認として、`supabase/checks/20260630_process_allowance_smoke_test.sql` を実行します。
このSQLはトランザクション内で前工程完了数が `2` の受注別工程を作成し、次工程へ `3` を登録しようとしてエラーになることを確認してから `ROLLBACK` します。
