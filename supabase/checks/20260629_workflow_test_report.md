...
製品工程マスタの工程順が飛び番でも、受注別工程を1からの連番に正規化するため、`supabase/migrations/20260701_normalize_order_process_sequence.sql` を追加。
既存テスト受注の補正SQLとして `supabase/checks/20260701_cross_screen_scenario_a_normalize_existing_order.sql` を追加。
...
2026-07-01 に画面横断シナリオA〜Eを実画面とSQLで確認。

確認結果:

- シナリオA: 受注登録、受注管理、注残管理、受注別工程管理の連携を確認。工程順の飛び番は `20260701_normalize_order_process_sequence.sql` と `20260701_cross_screen_scenario_a_normalize_existing_order.sql` で補正し、確認SQLは全行 `PASSED`。
- シナリオB: 実績登録、進捗管理、計量登録、計量表出力の連携を確認。計量実績は `production_results` に登録され、計量登録だけでは在庫が増えないことを確認。確認SQLは全行 `PASSED`。
- シナリオC: 梱包/包装実績登録後に在庫マスタへ登録または加算されることを確認。確認SQLは全行 `PASSED`。
- シナリオD: 在庫引当から出荷登録までの連携を確認。確認SQLは全行 `PASSED`。
- シナリオE: 注残管理からの削除後、工程、実績、生産予定、出荷、在庫引当が整理され、未出荷引当分の在庫引当数が戻ることを確認。確認SQLは全行 `PASSED`。

シナリオE確認後、削除済みまたは注残対象外の受注が生産予定に表示される問題を確認したため、作業系画面の表示基準を補正。

補正内容:

- 生産予定は、削除済み、最終工程完了済み、全数出荷済みの受注に紐づく予定を表示しない。
- 実績登録は、現在の注残対象に紐づく予定と工程だけを表示する。
- 受注別工程管理は、現在の注残対象に紐づく工程だけを表示する。
- `/api/daily-production` と `/api/production-schedules` は、注残対象外のデータを返さない。
...
- 削除済み受注が作業対象として表示されない
- 最終工程完了済み受注が作業対象として表示されない
- 全数出荷済み受注が作業対象として表示されない
