-- Make Gantt overlap days follow the current product process master value.
-- Existing order_processes keep their copied value, but the read view prefers
-- product_processes.overlap_days so master edits are reflected immediately.
...
select
  ...
  op.outsource_status,
  op.outsource_note,
  coalesce(pp.overlap_days, op.overlap_days, 0) as overlap_days
...
left join product_processes pp
  on pp.id = op.product_process_id;
